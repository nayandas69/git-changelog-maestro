...
from changelog_maestro.core.git import GitRepository, Commit, Tag
...
class TestGitRepository:
    ...
    def test_get_latest_tag(self, temp_repo: Path):
        ...
        try:
            ...
            assert commit2_date > commit1_date, f"Second commit ({commit2_date}) should be newer than first ({commit1_date})"
            ...
            assert latest_tag.name == "v1.1.0", f"Expected v1.1.0 but got {latest_tag.name}"
        finally:
            if git_repo:
                git_repo.close()
            repo.close()
